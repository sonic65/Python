...
class TestDemo():
    ...
    def test(self, test_data): #定义的测试函数，pytest框架内，以test_命名的函数被认为测试函数
        '''接口自动化测试----Test''' #这个文本会在报告头展示
        try:
            excleRow = test_data['rowIndex'] ###当前读取的excle行
            ###请求头
            with allure.step('开始发起请求：'+"{0}: {1}".format(str(test_data['method']), test_data['url'])): #报告中用来记录测试步骤
                ...
                commonMethod().write_excle(filepath,excleRow,5,res_to_json['code']) ###写入结果到excle
                commonMethod().write_excle(filepath,excleRow,6,res_to_json['message']) ###写入结果到excle
                ...
                issuccess = '失败'
                if res_to_json == test_data['except_code']: #如果断言成功，写入结果到excle
                    issuccess = '成功'
                commonMethod().write_excle(filepath, excleRow, 10, issuccess)
                assert res_to_json == test_data['except_code']###断言，用来判断当前测试是否通过
