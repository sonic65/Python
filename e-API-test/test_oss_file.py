    ...
    def test(self, test_data): 
        ...
        try:
            ...
            with allure.step('开始发起请求：'+"{0}: {1}".format(str(test_data['method']), test_data['url'])): 
                ...
                url = test_data['url']
                headers = None
                res = commonMethod().http_file_query(url,test_data['params'],test_data['method'],headers) ###发起请求                
                ...
                commonMethod().write_excle(filepath,excleRow,7,res) ###写入结果到excle
                ...
                assert res == test_data['except']###断言，用来判断当前测试是否通过
